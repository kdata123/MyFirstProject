...
public class HomeController {
	...
	public String index(Model model, HttpSession session) {
		...
		if(principal.toString().equals("anonymousUser")) {
			System.out.println("index 미인증");
			session.removeAttribute("mvo");
		}else {
			System.out.println("index 인증");
			UserDetails userDetails = (UserDetails)principal;
			Users users = (Users) memberService.loadUserByUsername(userDetails.getUsername());
			System.out.println("인증 정보 2 : " + users);
			session.setAttribute("mvo", users);
		}
		//-------------------------------------------------------------------------------------------
		*/
